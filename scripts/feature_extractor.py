...
#              Generate datasets
#################################################################################################################################
def split_urls_to_features(header):

    file = 'data/urls'  # Update the file name as needed

    db = pd.read_csv(file + '.csv')
    lst = list(db['url'])
    lst = list(set(lst))
    ...
    with open('data/features.csv', 'w') as csvfile:
        ...
        for url in lst:
            print(url)
            try:
                res = extract_features(url,status=None)
...
def generate_external_dataset(header,dataset_A):
    # db = pd.read_csv('data/dataset_A.csv')
    # lst = list(db['url'])
    # lst = list(set(lst))
    input_file = f'data/batches/{dataset_A}'
    output_file = 'data/dataset_B'  # Update the file name as needed

    if not os.path.isfile(output_file + '.csv'):
        with open(output_file + '.csv', 'w') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['url'] + header + ['status'])
    print(output_file)
    db = pd.read_csv(f"{output_file}.csv")
    lst = list(db['url'])
    lst = list(set(lst))

    dataset = pd.read_csv(f'{input_file}.csv')
    i = 0
    nb = 0
...
# split_urls_to_features(headers)
# generate_external_dataset(headers,'batch_2')